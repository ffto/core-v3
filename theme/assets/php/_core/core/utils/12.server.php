 ...
 * @return void
 */
function add_cron_job ($name, $time, $callback=null, $refresh=false){
	if (is_callable($time)){
		$refresh  = $callback;
		$callback = $time;
		$time     = $name;
	...
	$filename = _slug($name);
	$filepath = to_path('@env/cron/'.$filename);
	$now      = microtime(true);
	...
	if ($now > $expire || $refresh){
		...
		$time	= (int)$time;
		$expire = 'next update: ' . 465; //to_date($now, "update={$time}&format=:utc");
...
add_action('site-init', function ($set){
	...
	config_add_option('cookie_prefix', null, $cookie_default);
	config_add_option('session_cookie', null, true);
	config_add_option('session_lifetime', null, '30 minutes');
